...
/****** Object:  StoredProcedure [dbo].[spRPRT_CostReport_NewAcct2_Vision]    Script Date: 05/29/2013 09:23:06 ******/
...
LEFT JOIN  (
	SELECT
		SUM( bl.TotalDollars ) as total_dollars
		, bl.DeptGroup
	FROM DT_BudgetLines bl
	JOIN DT_Budgets b on bl.BudgetID = b.ID
	JOIN DT_Projects p on b.ProjectID = p.ID and p.Number = @Project
	GROUP BY DeptGroup
) budget_line_totals on budget_line_totals.DeptGroup = groups.Code

where 
	project.Project = @Project
	AND ISNULL(budget_line_totals.total_dollars,0) + ISNULL(budgets.Budget,0) + ISNULL(budgets.BudgetHrs,0) + ISNULL(actuals.TotalHrs,0)	+ ISNULL(actuals.Billed,0) + ISNULL(js.[BudgetHrs],0) + ISNULL(js.[RemainingHrs],0)	+ ISNULL(mp.[Projected],0) > 0
