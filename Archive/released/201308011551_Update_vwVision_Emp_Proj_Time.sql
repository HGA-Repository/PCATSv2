...
/****** Object:  View [dbo].[vwVision_Emp_Proj_Time]    Script Date: 08/01/2013 15:40:53 ******/
...
SELECT
	...
	, AcctCode = CASE 
		...
		WHEN LEN(v.[WBS3]) > 4 AND ISNUMERIC( LEFT(v.[WBS3],5) ) = 1 AND RIGHT(LEFT(v.[WBS3],5), 1) <> '.' then LEFT(v.[WBS3],5)
		WHEN ISNUMERIC( LEFT(v.[WBS3],4) ) = 1 AND RIGHT(LEFT(v.[WBS3],4), 1) <> '.' then 
	...
	,GroupCode = CASE 
		...
		WHEN LEN(v.[WBS3]) > 4 AND ISNUMERIC( LEFT(v.[WBS3],5) ) = 1 AND RIGHT(LEFT(v.[WBS3],5), 1) <> '.' then ((LEFT(v.[WBS3],5) / 1000) * 1000 )
		WHEN ISNUMERIC( LEFT(v.[WBS3],4) ) = 1 AND RIGHT(LEFT(v.[WBS3],4), 1) <> '.' then 
