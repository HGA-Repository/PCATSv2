...
/****** Object:  StoredProcedure [dbo].[spRPRT_CostReport_NewAcct2_Vision]    Script Date: 2/6/2016 11:08:01 AM ******/
SET ANSI_NULLS ON
GO
...
alter PROCEDURE [dbo].[spRPRT_CostReport_NewAcct2_Vision_TestOnFeb]
	@Project	varchar(50),
...
LEFT JOIN(
   ...
   (
	...
	--WHERE [ForecastHrs] >= 0 or [ForecastAmnt] is not null
	GROUP BY f.code

) forecast on forecast.code = tasks.Code
WHERE (budget > 0 OR costs > 0) or (costs < 0)
