...
/****** Object:  StoredProcedure [dbo].[spBudgetLine_ListByBudget]    Script Date: 04/24/2013 16:20:56 ******/
...
FROM
	DT_BudgetLines bl
	JOIN (SELECT * FROM vwAccountCodes) ac ON bl.[Task] = ac.[Task] AND bl.[Category] = ac.[Category] AND bl.[Activity] = ac.[Activity]
WHERE
	bl.[Deleted] = 0
	AND bl.[BudgetID] = @BudgetID
	AND bl.[DeptGroup] = @GroupID
	AND bl.[WBS] LIKE @WBS + '%'
